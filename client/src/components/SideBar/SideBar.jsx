...
//         setMenuPosition({
//           top: folderRect.bottom - folderListRect.top + 180,
//           left: folderRect.right - folderListRect.left - 40,
//         });
//       } else {
//         setMenuPosition({
//           top: folderRect.top - folderListRect.top + 110,
//           left: folderRect.right - folderListRect.left - 30,
//         });
//       }
//     }
//   };
...
//             <Link to="/">
//               <div
//                 className={`folder-item relative flex flex-row items-center justify-start bg-newNotes-sidebar py-2 ${location.pathname === "/" ? "bg-stone-200" : "hover:bg-stone-200"} transition-all  w-12/12 space-x-1 px-3 truncate text-sm font-medium`}
//               >
//                 <div>
//                   <HomeIcon color={"black"} size={18} />
//                 </div>
...
//             <Link to="/search">
//               <div
//                 className={`folder-item relative flex flex-row items-center justify-start bg-newNotes-sidebar py-2 ${location.pathname === "/search" ? "bg-stone-200" : "hover:bg-stone-200"} transition-all  w-12/12 space-x-1 px-3 truncate text-sm font-medium`}
//               >
//                 <div>
//                   <SearchIcon color={"black"} size={15} />
//                 </div>
...
//           <div
//             ref={folderListRef}
//             className={`flex-1 overflow-y-auto relative pb-0 ${isCollapsed ? "max-h-[calc(86vh-100px)]" : "max-h-[calc(102vh-250px)]"} sm:max-h-${isCollapsed ? "[calc(80vh-80px)]" : "[calc(80vh-100px)]"} md:max-h-${isCollapsed ? "[calc(90vh-110px)]" : "[calc(90vh-120px)]"} lg:max-h-${isCollapsed ? "[calc(86vh-120px)]" : "[calc(86vh-128px)]"} xl:max-h-${isCollapsed ? "[calc(90vh-140px)]" : "[calc(90vh-140px)]"} 2xl:max-h-${isCollapsed ? "[calc(92vh-150px)]" : "[calc(92vh-160px)]"} 3xl:max-h-${isCollapsed ? "[calc(94vh-160px)]" : "[calc(94vh-170px)]"} 4xl:max-h-${isCollapsed ? "[calc(96vh-170px)]" : "[calc(96vh-180px)]"}`}
//           >
//             {sortedFolders && sortedFolders.length > 0 ? (
//               sortedFolders.map((folder) => (
//                 <Link
//                   to={`/folder/${folder._id}`}
//                   key={folder._id}
//                   className={`folder-item relative flex items-center justify-between bg-newNotes-sidebar  py-2 ${location.pathname === `/folder/${folder._id}` ? "bg-stone-200" : "hover:bg-stone-200"} transition-all  w-12/12 space-x-1 px-3 `}
//                 >
//                   <div className="flex align-middle w-11/12 truncate ">
//                     <span className=" truncate text-sm font-medium  ">
//                       {folder.name}
...
import HomeIcon from "../SvgIcons/HomeIcon.jsx";
import ArchivedIcon from "../SvgIcons/ArchivedIcon.jsx";
import ErrorComponent from "../../utils/ErrorComponent.jsx"; // Import the ErrorComponent
import {
  fetchAllFolders,
  createFolder,
  editFolder,
...
const SideBar = ({ isCollapsed, toggleCollapse }) => {
  ...
  const confirmDeleteFolder = () => {
    dispatch(removeFolder(folderToDelete._id)).then((action) => {
      if (removeFolder.fulfilled.match(action)) {
        setShowDeleteConfirmation(false);
        setFolderToDelete(null);
        setActiveMenu(null);
        navigate("/"); // Redirect to home page
      } else {
        setErrorMessage("Failed to delete folder"); // Set error message
        setTimeout(() => {
          setErrorMessage(null); // Clear error message after some time
          navigate("/"); // Redirect to home page
  ...
  return (
    <div
      className={`fixed left-0 top-0 h-full ${isCollapsed ? " flex item-center w-auto   bg-white bg-opacity-0" : "w-custom-width"}  bg-gradient-to-r from-gray-50 to-gray-50 text-gray-800 flex flex-col  transition-all duration-300`}
    >
      {isCollapsed && (
        <div className="flex flex-col items-center flex-1 space-y-2 ">
          {/* <div className="flex items-center justify-center space-x-2 m-2.5">
            <button
              onClick={toggleCollapse}
              className="p-1 hover:bg-stone-200 rounded"
            >
      ...
      {!isCollapsed && (
        <div className="flex flex-col flex-1 ">
          <div className="flex items-center justify-between my-3 mx-2.5 ">
            <button
              onClick={toggleCollapse}
              className=" p-1 hover:bg-stone-200 rounded"
            >
              <SidebarRectangleIcon color={"#666666"} size={25} />
            </button>
          ...
          <div>
            <Link to="/">
              <div
                className={`folder-item relative flex flex-row items-center justify-start bg-newNotes-sidebar  py-2 ${location.pathname === "/" ? "bg-stone-200" : "hover:bg-stone-200"} transition-all  w-12/12 space-x-1 px-3.5 truncate text-sm font-medium`}
              >
                <div>
                  <HomeIcon color={"black"} size={18} />
                </div>
            ...
            <Link to="/search">
              <div
                className={`folder-item relative flex flex-row items-center justify-start bg-newNotes-sidebar py-2  ${location.pathname === "/search" ? "bg-stone-200" : "hover:bg-stone-200"} transition-all  w-12/12 space-x-1 px-3.5 truncate text-sm font-medium`}
              >
                <div>
                  <SearchIcon color={"black"} size={15} />
                </div>
          ...
          <div className="flex flex-row justify-between w-full px-3 py-1 text-sm  ">
            <button
              onClick={() => setSortMethod("recent")}
              className={`px-2 py-1 border border-stone-200 text-gray-800 font-semibold rounded-md hover:bg-stone-300 transition-all ${sortMethod === "recent" ? "bg-stone-300 text-black" : ""}`}
            >
              Recent
            </button>
            <button
              onClick={() => setSortMethod("oldest")}
              className={`px-2 py-1  border border-stone-200 text-gray-800 font-semibold rounded-md hover:bg-stone-300 transition-all ${sortMethod === "oldest" ? "bg-stone-300 text-black" : ""}`}
            >
              Oldest
            </button>
            <button
              onClick={() => setSortMethod("atoz")}
              className={`px-2 py-1  border border-stone-200 text-gray-800 font-semibold rounded-md hover:bg-stone-300 transition-all ${sortMethod === "atoz" ? "bg-stone-300 text-black" : ""}`}
            >
              A to Z
            </button>
          </div>
          ...
          <div
            ref={folderListRef}
            className={`flex-1 overflow-y-auto relative pb-0 
              ${isCollapsed ? "max-h-[calc(86vh-100px)]" : "max-h-[calc(102vh-250px)]"} 
              sm:max-h-${isCollapsed ? "[calc(80vh-80px)]" : "[calc(80vh-100px)]"} 
              md:max-h-${isCollapsed ? "[calc(90vh-110px)]" : "[calc(90vh-120px)]"} 
              lg:max-h-${isCollapsed ? "[calc(86vh-120px)]" : "[calc(86vh-128px)]"} 
            ...
            {sortedFolders && sortedFolders.length > 0 ? (
              sortedFolders.map((folder) => (
                <Link
                  to={`/folder/${folder._id}`}
                  key={folder._id}
                  className={`folder-item relative flex items-center justify-between bg-newNotes-sidebar mb-0.5 py-2 ${location.pathname === `/folder/${folder._id}` ? "bg-stone-200" : "hover:bg-stone-200"} transition-all  w-12/12 space-x-1 px-3 `}
                >
                  <div className="flex align-middle w-11/12 truncate ">
                    <span className=" truncate text-sm font-medium  ">
                      {folder.name}
